...
public:
		...
		: Layer("Example"), m_Camera(-1.6f, 1.6f, -0.9f, 0.9f), m_CameraPosition(0.0f)
	{
		...
		std::string fragmentSrc = R"(
			...
			void main()
			{
				color = vec4(v_Position * 0.5 + 1.0, 1.0);
				color = v_Color;
			}
		)";
	...
	{
		if (Engine::Input::IsKeyPressed(ENGN_KEY_LEFT))
			m_CameraPosition.x -= m_CameraMoveSpeed * ts;
		else if (Engine::Input::IsKeyPressed(ENGN_KEY_RIGHT))
			m_CameraPosition.x += m_CameraMoveSpeed * ts;

		if (Engine::Input::IsKeyPressed(ENGN_KEY_UP))
			m_CameraPosition.y += m_CameraMoveSpeed * ts;
		else if (Engine::Input::IsKeyPressed(ENGN_KEY_DOWN))
			m_CameraPosition.y -= m_CameraMoveSpeed * ts;

		if (Engine::Input::IsKeyPressed(ENGN_KEY_A))
			m_CameraRotation += m_CameraRotationSpeed * ts;
		if (Engine::Input::IsKeyPressed(ENGN_KEY_D))
			m_CameraRotation -= m_CameraRotationSpeed * ts;
